...
if($id){
	$filtercategory=$id?"join products where products.product_size=product_size.id and products.category_id='".$id."'":'';
}
	
$cagegory_tablename="select product_size.id, product_size.size from product_size  $filtercategory group by product_size.id";
$pro_categories=$db->find($cagegory_tablename);
return 	$pro_categories;
}
