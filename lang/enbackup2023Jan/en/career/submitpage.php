...
set_error_handler("var_dump");*/
	...
	if ($_POST["submit"]) {
		...
		$phone = $_POST['phone'];
		$from = $_POST['fname'];     
		$to = 'HR@dynamic-test.com';
		//$to = 'srini@dynamic-test.com';
		//$to = 'manoj.kalai@gmail.com';
		//$to = 'kartik.frendz@gmail.com'; 	   //dreammakerz.sugumar@gmail.com,HR@dynamic-test.com,salesca@dynamic-test.com,petea@dynamic-test.com
		...
		if (!$_POST['jobtitle']) {
			$errJobTitle = 'Position Applied should not be null';
		}
		
		if (!$_POST['ctry']) {
			$errCountry = 'Country Applied should not be null';
		}
		...
		// create a boundary string. It must be unique
		  ...
		  $message.="Email: ".$email."\n";
		  $message.="Position Applied For: ".$jobtitle."\n";
		...
		//echo "fname=".$errFname." email=".$errEmail." ph=".$errPhone." empty= ".$errempty." size=".$errsize." title= ".$errJobTitle." country= ".$errCountry;
		//exit();
// If there are no errors, send the email
if (!$errFname && !$errEmail && !$errPhone && !$errempty && !$errsize && !$errJobTitle && !$errCountry)
	
	{
//mail($to,$subject,$message);
	//print_r(error_get_last());
	//die("enter Email loop");
	//echo "test0";
		//exit;
	//echo "to=".$to."sub=".$subject." mes=".$message;
		//exit();
	if (mail($to,$subject,$message,$headers)) {
		...
		//$result='<div class="alert alert-success">Thank you for applying for the career opportunity at DTS!</div>';
		...
		//header( "refresh:1;url=http://dynamic-test.com/sucess.php");
	...
	if($_POST['fname']!='' && $_POST['email']!='' && $_POST['phone']!='' && $_FILES['file']['name']!='' && $_POST['jobtitle']!='' && $_POST['ctry']!='' ){
		//echo "test3";
		//exit;
		$url='http://dynamic-test.com/sucess.php';
		echo '<script>window.location = "'.$url.'";</script>';
  }
	else {
		
	echo '<script>window.location = submitpage.php?t="'.$_GET[t].'"&c="'.$_GET['c'].'";</script>';
	}
}
?>
...
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-123333665-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-123333665-1');
</script>
  ...
  <div class="container">
    <div class="row">
      ...
      <div class="span12 left0">
        ...
        <div class="span6">
          <form class="enquiry-form" role="form" method="post" onsubmit="return Validate(this);" enctype="multipart/form-data">
            ...
            <div class="form-group">
              <label for="email" class="col-sm-2 control-label">Position Applied For<span class="star-fields">*</span></label>
              <div class="col-sm-10">
                <input type="text" class="form-control" id="position" name="position"  disabled value="<?php echo htmlspecialchars($_GET['t']); ?>"><?php echo "<p class='text-danger'>$errJobTitle</p>";?> 
               </div>
            </div>
            ...
            <div class="span5">
            <div class="form-group">
              <label for="email" class="col-sm-2 control-label">Country Applied For<span class="star-fields">*</span></label>
              <div class="col-sm-10">
                <input type="text" class="form-control" id="country" name="country"  disabled value="<?php echo htmlspecialchars($_GET['c']); ?>"><?php echo "<p class='text-danger'>$errCountry</p>";?> 
               </div>
            </div> 
            <div class="form-group">
            ...
            <div class="form-group">
              ...
              <div class="col-sm-10">
                ...
                <input type="hidden" name="ctry" value="<?php echo $_GET['c']; ?>" />
                 <input type="hidden" name="jobtitle" value="<?php echo $_GET['t']; ?>" />
      ...
      <div class="col-sm-12 col-sm-offset-2"> <?php echo $result; ?> </div>
      <!--<div class="span4">
        <div class="pad20px_l">
          <div >
